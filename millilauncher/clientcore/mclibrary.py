...
The universal file exists and is accessible in Mojang's server, but is empty.
...
class MCLibrary:
    def __init__(self, d):
        ...
        if self.allow:
            # self.url, self.path
            native_key = Template(d['natives'][dirs.system]).substitute(
                arch=dirs.arch) if 'natives' in d else None
        ...
        if 'rules' in d:
            for action_dict in d['rules']:  # d['rules'] is a list of dicts
                ...
                if action_dict['action'] == 'allow' and 'os' in action_dict:
                    if dirs.system == action_dict['os']['name']:
                        pattern = action_dict['os'].get('version')
                        if pattern:
                            allow = re.match(pattern, dirs.version)
                        else:
                            allow = True
                    else:
                        allow = False
                elif action_dict['action'] == 'disallow':
                    allow = (dirs.system != action_dict['os']['name'])
        return allow
    ...
    def _parse_name(name, native):
        ...
        if native:
            return '{0}/{1}/{2}/{1}-{2}-{3}.jar'.format(
                package.replace('.', '/'), name, version, native)
        else:
            return '{0}/{1}/{2}/{1}-{2}.jar'.format(
