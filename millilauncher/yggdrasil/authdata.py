...
class AuthUserData:
    ...
    def __init__(self, data, **kwargs):
        if isinstance(data, yggdrasil.request.Response):
            self.access_token = data['accessToken']
            self.username = kwargs.get('username')
            self.properties = AuthUserProperties(data['user']['properties'])
            self.userid = data['user']['id']
            profile = data['selectedProfile']
            self.profile = AuthProfileData(
                profile['name'], profile['id'], profile.get('legacy', False))
            ...
            if format in (20, 21):
                ...
                self.profile = AuthProfileData(
                    profile[1]['displayName'], profile[0],
                    profile[1].get('legacy', False))
            else:  # 18 or 17
                self.access_token = data['accessToken']
                self.username = data['username']
                ...
                self.profile = AuthProfileData(
                    data['displayName'], id, data.get('legacy', False))
...
class AuthenticationDatabase:
    def __init__(self, data):
        ...
        if self.format < 20:
            ...
            # for x, y in data['authenticationDatabase'].items():
            #     self.users[y['userid']] = AuthUserData(y, id=x, format=self.format)
        elif self.format in (20, 21):
            self.selected_id = data['selectedUser']['account']
            self.users = {x: AuthUserData(y, id=x, format=self.format)
                          for x, y in data['authenticationDatabase'].items()}
        else:
